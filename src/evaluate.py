...
# Métricas antigas (general) — mantidas como backup
# from metrics import evaluate_f1_score, evaluate_clarity, evaluate_precision

# Métricas específicas do desafio Bug-to-User-Story (conforme README)
from metrics import (
    evaluate_tone_score,
...
def evaluate_prompt(
    ...
    try:
        ...
        # --- CÓDIGO ANTIGO (métricas gerais) — backup ---
        # f1_scores = []
        # clarity_scores = []
        # precision_scores = []
        #
        ...
        # Métricas específicas do desafio (conforme README)
        tone_scores = []
        acceptance_scores = []
        format_scores = []
        completeness_scores = []
        ...
        for i, example in enumerate(examples[:10], 1):
            ...
            if result["answer"]:
                ...
                print(f"      [{i}/{min(10, len(examples))}] Tone:{tone['score']:.2f} AC:{acceptance['score']:.2f} Format:{fmt['score']:.2f} Completeness:{completeness['score']:.2f}")
...
def display_results(prompt_name: str, scores: Dict[str, float]) -> bool:
    ...
    print("\nMétricas do Desafio (conforme README):")
    ...
    # Conforme README: cada métrica deve ser >= 0.9
    ...
    if passed:
        ...
        print(f"⚠️  Média atual: {average_score:.4f} | Necessário: cada métrica >= 0.9000")
